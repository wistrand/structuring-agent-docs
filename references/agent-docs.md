...
A generated file opens with a marker so no one hand-edits it:

```markdown
<!-- GENERATED by `make settings-doc` from src/settings.schema.json.
...
|----------------|--------|---------|---------------------------|
| `cache.ttlSec` | number | `300`   | Cache lifetime in seconds |
```
